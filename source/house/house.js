...
function getMonthlyDetail({
    ...
    const totalMonths = Math.ceil(loanYears * 12)
    ...
    aheadPayMoney && console.table(aheadMonthlyData);
...
function getMonthlyDetailForSameBenJin({
  firstPayMoney, // 首付
  loanMoney,
  loanRate,
  loanYears,
  ...
  let monthlyData = [];
  let remainBenjin = loanMoney;
  let monthlyBenjin = remainBenjin;
  ...
  const getMonthlyInfo = () => {
    while (remainBenjin > 0) {
      ...
      const monthlyInterest = remainBenjin * monthlyRate;
      const monthlyPay = monthlyBenjin  + monthlyInterest
  ...
  while (monthlyData.length != totalMonths) {
    ...
    const isSmall = actualTotalMonths > totalMonths;
    const ls = isSmall ? smallLs : bigLs;
    ls.push(monthlyBenjin);
    let delta = 1;
    if (isSmall) {
      const lastBig = bigLs[bigLs.length - 1];
      delta = lastBig ? (lastBig - monthlyBenjin) * 0.5 : monthlyBenjin * 0.5;
    } else {
      const lastSmall = smallLs[smallLs.length - 1];
      delta = lastSmall
        ? (lastSmall - monthlyBenjin) * 0.5
        : (0 - monthlyBenjin) * 0.5;
    }
    // const delta = actualTotalMonths > totalMonths ? 1 : -1;
    monthlyData = [];
    remainBenjin = loanMoney;
    monthlyBenjin = monthlyBenjin + delta;
    const minMonthlyPay = remainBenjin * monthlyRate;
    if (monthlyBenjin * (1 + monthlyRate) < minMonthlyPay) {
      monthlyBenjin = monthlyBenjin + 100;
    }
    console.log("🚀 ~ getMonthlyDetail ~ monthlyBenjin:", monthlyBenjin);
    console.time("mm");
    getMonthlyInfo();
    console.timeEnd("mm");
  }
  ...
  return {
    ...
    aheadMonthlyData,
    monthPayMoney,
    monthlyBenjin: toFixed(monthlyBenjin),
    endTime,
    aheadEndTime,
    hasPayLixi,
    hasPayAmount,
...
window.myapp = new Vue({
  ...
  data() {
    const qyInfo = {
      nowHouseAmount: 880000,
      firstPayRate: 0.3,
      firstPayMoney: 321600,
      loanRate: 0.05,
      loanYears: 13,
      aheadPayMoney: 0,
    ...
    return { baseInfo: qyInfo, winWidth, scrollLeft: 0, headerTableHeight: 69 }
  },
  computed: {
    house() {
      console.log('----------> computed house data');
  ...
  methods: {
    ...
    getData: ({
      ...
      const getMonthlyDetailMethod = payType === SAME_BENJIN ? getMonthlyDetailForSameBenJin : getMonthlyDetail;
      const detail = getMonthlyDetailMethod({
        firstPayMoney,
        loanMoney,
        loanRate,
        loanYears,
